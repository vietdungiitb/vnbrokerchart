...
class AxisZoomCapture extends Component<any, any> {
	node: any;
	...
	handleDragStartTouch(e: any) {
		...
		if (e.touches.length === 1 && startScale.invert) {
			...
			const startXY = pointer(e, this.node);
	...
	handleDrag(e: any) {
		...
		if (isDefined(startPosition)) {
			...
			const center = mean(startScale.range()) ?? 0;

			const tempRange = startScale.range()
				.map((d: any) => inverted ? d - sign(d - center) * diff : d + sign(d - center) * diff);

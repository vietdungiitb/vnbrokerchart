...
const GenericChartComponentAny: any = GenericChartComponent;
...
class Axis extends Component<any, any> {
	...
	render() {
		...
		return <g transform={`translate(${ transform[0] }, ${ transform[1] })`}>
			{zoomCapture}
			<GenericChartComponentAny ref={this.saveNode}
...
function tickHelper(props: any, scale: any) {
	const {
	...
	let ticks: any, dy: any, canvas_dy: any, textAnchor: any;
	...
	if (orient === "bottom" || orient === "top") {
		...
		if (showTicks && flexTicks) {
			const nodes = ticks.map((d: any) => ({ id: d.value, value: d.value, fy: d.y2, origX: d.x1 }));

			const simulation = forceSimulation(nodes)
				.force("x", forceX((d: any) => d.origX).strength(1))
...
function axisLineSVG(props: any, range: any) {
...
function drawAxisLine(ctx: any, props: any, range: any) {
...
function axisTicksSVG(props: any, scale: any) {
...
function drawTicks(ctx: any, result: any) {
...
function drawEachTick(ctx: any, tick: any, result: any) {
...
function drawEachTickLabel(ctx: any, tick: any, result: any) {

...
function tickTransform_svg_axisX(scale: any, tick: any) {
	return [Math.round(scale(tick)), 0];
}

function tickTransform_svg_axisY(scale: any, tick: any) {
	return [0, Math.round(scale(tick))];
}

class Tick extends Component<any, any> {
	render() {
...
(Tick as any).drawOnCanvasStatic = (tick: any, ctx: any, result: any) => {
...
class AxisTicks extends Component<any, any> {
	static helper: any;
...
AxisTicks.helper = (props: any, scale: any) => {
	...
	const format = isNotDefined(tickFormat)
		? baseFormat
		: (d: any) => baseFormat(d) ? tickFormat(d) : "";

	const sign = orient === "top" || orient === "left" ? -1 : 1;
	const tickSpacing = Math.max(innerTickSize, 0) + tickPadding;

	let tickTransform: any, x: any, y: any, x2: any, y2: any, dy: any, canvas_dy: any, textAnchor: any;
...
AxisTicks.drawOnCanvasStatic = (props: any, ctx: any, xScale: any, yScale: any) => {
	...
	result.ticks.forEach((tick: any) => {
		(Tick as any).drawOnCanvasStatic(tick, ctx, result);
	});
};

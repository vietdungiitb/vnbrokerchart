...
class VolumeProfileSeries extends Component<any, any> {
	...
	renderSVG(moreProps: any) {
		...
		const sessionBgSvg = showSessionBackground
			? sessionBg.map((d: any, idx: number) => <rect key={idx} {...d} opacity={sessionBackGroundOpacity} fill={sessionBackGround} />)
			: null;

		return <g className={className}>
			{sessionBgSvg}
			{rects.map((d: any, i: number) => <g key={i}>
				<rect x={d.x} y={d.y}
...
function helper(props: any, moreProps: any, xAccessor: any, width: number) {
	...
	const allRects = sessions.map((session: any[]) => {
		...
		const directionComparator = orient === "right" ? descending : ascending;

		const values = histogram2(session);

		const volumeInBins = values
			.map(arr => arr.map((d: any) => absoluteChange(d) > 0 ? { direction: "up", volume: volume(d) } : { direction: "down", volume: volume(d) }))
			.map(arr => {
				const totalsByDirection = arr.reduce((acc: any, item: any) => {
					acc.set(item.direction, (acc.get(item.direction) || 0) + item.volume);
					return acc;
				}, new Map());

				return Array.from(totalsByDirection.entries())
					.sort((a: any, b: any) => directionComparator(a[0], b[0]))
					.map((entry: any) => ({ key: entry[0], value: entry[1] }));
			});

		const volumeValues = volumeInBins
			.map(each => sum(each.map((d: any) => d.value)));

		const base = (xScale: any) => head(xScale.range());
		...
		const xScale = scaleLinear()
			.domain([0, max(volumeValues) ?? 0])
			.range([start, end]);

		const totalVolumes = volumeInBins.map(volumes => {

			const totalVolume = sum(volumes, (d: any) => d.value);
			const totalVolumeX = xScale(totalVolume);
			const widthValue = base(xScale) - totalVolumeX;
			const x = widthValue < 0 ? totalVolumeX + widthValue : totalVolumeX;

			const ws = volumes.map((d: any) => {
				return {
					type: d.key,
					width: d.value * Math.abs(widthValue) / totalVolume,
				};
			});
		...
		const rects = (zip as any)(values, totalVolumes)
			.map((pair: any) => {
				const d = pair[0];
				const x = pair[1].x;
				const ws = pair[1].ws;
	...
	return {
		rects: merge(allRects.map((d: any) => d.rects)),
		sessionBg: allRects.map((d: any) => d.sessionBg),
	};
}


function drawOnCanvas(ctx: any, props: any, rects: any[], sessionBg: any[]) {

...
class PointAndFigureSeries extends Component<any, any> {
			...
			<g className={className}>
				{columns.map((col: any, idx: number) => (
					<g key={idx} className={col.className} transform={`translate(${ col.offset[0] }, ${ col.offset[1] })`}>
						{col.boxes.map((box: any, i: number) => {
							if (col.direction > 0) {
								return (
									<g key={`${ idx }-${ i }`}>
...
function drawOnCanvas(ctx: any, props: any, columns: any[]) {
...
function getColumns(xScale: any, xAccessor: any, yScale: any, plotData: any[]) {
	...
	const columns = plotData
		.filter((d: any) => isDefined(d.close))
		.map((d: any) => {

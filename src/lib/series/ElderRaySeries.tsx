...
class ElderRaySeries extends Component<any, any> {
	...
	yAccessorForBarBase(xScale: any, yScale: any, d: any) {
		const { yAccessor } = this.props;
		const y = yAccessor(d) && Math.min(yAccessor(d).bearPower, 0);
		return yScale(y);
	}
	fillForEachBar(d: any, yAccessorNumber: number) {

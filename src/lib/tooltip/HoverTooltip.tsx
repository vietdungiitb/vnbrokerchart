...
class HoverTooltip extends Component<any, any> {
	...
	renderSVG(moreProps: any) {
		...
		if (isNotDefined(pointer)) return null;
		const safePointer = pointer as NonNullable<typeof pointer>;

		const { bgFill, bgOpacity, backgroundShapeSVG, tooltipSVG } = this.props;
		const { bgheight, bgwidth } = this.props;
		const { height } = moreProps;

		const { x, y, content, centerX, pointWidth, bgSize } = safePointer;
...
HoverTooltip.contextType = StockChartContext;
...
function backgroundShapeSVG({ fill, stroke, opacity }: any, { height, width }: any) {
	return <rect
		height={height}
		width={width}
		fill={fill}
...
function normalizeX(x: number, bgSize: any, pointWidth: number, width: number) {
	return x < width / 2
		? x + pointWidth / 2 + PADDING
		: x - bgSize.width - pointWidth / 2 - PADDING;
}

function normalizeY(y: number, bgSize: any) {
	return y - bgSize.height <= 0
		? y + PADDING
		: y - bgSize.height - PADDING;
}

function origin(props: any, moreProps: any, bgSize: any, pointWidth: number) {

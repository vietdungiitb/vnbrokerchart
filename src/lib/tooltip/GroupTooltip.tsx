...
class SingleTooltip extends Component<any, any> {
...
class GroupTooltip extends Component<any, any> {
	static defaultProps: any;
	...
	renderSVG( moreProps: any ) {
		...
		const groupTextAnchor = textAnchor == null ? undefined : (textAnchor as "start" | "middle" | "end" | "inherit");

		const singleTooltip = options.map( ( each: any, idx: number ) => {
		...
		return (
			<g transform={`translate(${xPos}, ${yPos})`} className={className} textAnchor={groupTextAnchor as any}>

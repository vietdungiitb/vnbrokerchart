...
class FibonacciRetracement extends Component<any, any> {
	[key: string]: any;
	...
	handleDrawRetracement(xyValue: any) {
	...
	handleDrag(index: number, xy: any) {
		this.setState({
			override: {
				index,
				...xy
	...
	handleEdge1Drag(echo: any, newXYValue: any, origXYValue: any) {
	...
	handleEdge2Drag(echo: any, newXYValue: any, origXYValue: any) {
		const { retracements } = this.props;
		const { index } = echo;
	...
	handleStart(xyValue: any, moreProps: any) {
		const { current } = this.state;
	...
	render() {
		...
		return (
			<g>
				{retracements.map((each: any, idx: number) => {

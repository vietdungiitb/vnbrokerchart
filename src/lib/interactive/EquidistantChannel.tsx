...
class EquidistantChannel extends Component<any, any> {
	[key: string]: any;
	...
	handleDragChannel(index: number, newXYValue: any) {
		this.setState({
			override: {
				index,
				...newXYValue
	...
	handleDragChannelComplete(moreProps: any) {
		...
		if (isDefined(override)) {
			...
			const newChannels = channels
				.map((each: any, idx: number) => idx === index
	...
	handleEnd(xyValue: any, moreProps: any, e: any) {
		...
		if (this.mouseMoved
			...
			if (isNotDefined(current.dy)) {
				...
				const newChannels = [
					...channels.map((d: any) => ({ ...d, selected: false })),
					{
	...
	render() {
		...
		return <g>
			{channels.map((each: any, idx: number) => {

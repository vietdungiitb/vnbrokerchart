...
class EachGannFan extends Component<any, any> {
	[key: string]: any;
	...
	handleLine1Edge1Drag(moreProps: any) {
		const { index, onDrag } = this.props;
		const {
			startXY,
		} = this.dragStart;
	...
	handleLine1Edge2Drag(moreProps: any) {
		const { index, onDrag } = this.props;
		const {
			endXY,
		} = this.dragStart;
	...
	handleChannelHeightChange(moreProps: any) {
		const { index, onDrag } = this.props;
	...
	getEdgeCircle({ xy, dragHandler, cursor, fill, edge }: any) {
...
EachGannFan.defaultProps = {
	yDisplayFormat: (d: any) => d.toFixed(2),
	interactive: true,
	selected: false,

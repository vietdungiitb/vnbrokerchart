...
class EachFibRetracement extends Component<any, any> {
	[key: string]: any;
	...
	handleLineNSResizeTop(moreProps: any) {
		const { index, onDrag } = this.props;
		const {
			x1, x2, y2,
		} = this.props;
	...
	handleLineNSResizeBottom(moreProps: any) {
		const { index, onDrag } = this.props;
		const {
			x1, y1, x2,
		} = this.props;

...
class TrendLine extends Component<any, any> {
	[key: string]: any;
	...
	handleDragLine(index: number, newXYValue: any) {
		this.setState({
			override: {
				index,
				...newXYValue
	...
	handleDragLineComplete(moreProps: any) {
		const { override } = this.state;
		if (isDefined(override)) {
			const { trends } = this.props;
			const newTrends = trends
				.map((each: any, idx: number) => idx === override.index
					? {
						...each,
	...
	handleStart(xyValue: any, moreProps: any, e: any) {
		const { current } = this.state;
	...
	handleEnd(xyValue: any, moreProps: any, e: any) {
		const { current } = this.state;
		...
		if (this.mouseMoved
			...
			const newTrends = [
				...trends.map((d: any) => ({ ...d, selected: false })),
				{
					start: current.start,
					end: xyValue,
					selected: true,
	...
	render() {
		...
		return <g>
			{trends.map((each: any, idx: number) => {
...
TrendLine.defaultProps = {
	...
	shouldDisableSnap: (e: any) => (e.button === 2 || e.shiftKey),
	hoverText: {

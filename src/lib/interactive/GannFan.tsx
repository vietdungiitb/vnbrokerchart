...
class GannFan extends Component<any, any> {
	[key: string]: any;
	...
	handleDragFan(index: number, newXYValue: any) {
		this.setState({
			override: {
				index,
				...newXYValue
	...
	handleDragFanComplete(moreProps: any) {
	...
	handleEnd(xyValyue: any, moreProps: any, e: any) {
		const { current } = this.state;
		...
		if (this.mouseMoved
			...
			const newfans = [
				...fans.map((d: any) => ({ ...d, selected: false })),

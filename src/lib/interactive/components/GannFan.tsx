...
class GannFan extends Component<any, any> {
	...
	drawOnCanvas(ctx: CanvasRenderingContext2D, moreProps: any) {
		...
		pairsOfLines.forEach(([line1, line2]: any, idx: number) => {
	...
	renderSVG(moreProps: any) {
		...
		return (
			<g>
				...
				{pairsOfLines.map(([line1, line2]: any, idx: number) => {
					const ctx = d3Path();
					ctx.moveTo(line1.x1, line1.y1);
					ctx.lineTo(line1.x2, line1.y2);
					ctx.lineTo(line2.x2, line2.y2);
...
function getLineCoordinates(start: any, endX: any, endY: any, text: any) {
	const end = [
		endX,
		endY
	];
...
function helper(props: any, moreProps: any) {
	...
	if (dx !== 0 && dy !== 0) {
		...
		const lineCoods = lines.map((line: any) => {

...
class MouseLocationIndicator extends Component<any, any> {
	[key: string]: any;
	...
	handleMouseDown(moreProps: any, e: any) {
		const pos: any = this.xy(moreProps, e);
		if (isDefined(pos)) {
	...
	handleClick(moreProps: any, e: any) {
		const pos: any = this.xy(moreProps, e);
		if (isDefined(pos)) {
			const { xValue, yValue, x, y } = pos;
			this.mutableState = { x, y };
	...
	handleMousePosChange(moreProps: any, e: any) {
		if (!shallowEqual((moreProps as any).mousXY, (moreProps as any).prevMouseXY)) {
			const pos: any = this.xy(moreProps, e);
			if (isDefined(pos)) {
				const { xValue, yValue, x, y } = pos;
				this.mutableState = { x, y };
